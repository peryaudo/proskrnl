...
#
#   winetest-gui 114  winetest 78  wow64gui 76  gui5con 72  tornwrite 66  gui4 62
...
#
# 1165 s of legs; 150 s of wall clock. The floor is winetest-gui at 114 s, so the
...
ALL_LEGS=(winetest-gui winetest wow64gui wow64aud winefbunit gui5con gui6 coldinput usbinput liveusb liveusbuefi usbkbd usbmouse tornwrite gui4 cui8 gui3 gui5 gui2 cui9
...
run_leg() {
    ...
    *)
        ...
        boot)
            ...
            ;;
        gui2|gui3|gui4|gui5|gui5con|wow64gui|usbinput|usbkbd|usbmouse)
            export TIMEOUT=1200 GUI_DEADLINE=900
            cmd=("$view/tests/run/run.sh" "$leg")
            ;;
        liveusb|liveusbuefi)
            export TIMEOUT=1800 EXPECT_DEADLINE=1500
            cmd=("$view/tests/run/run.sh" "$leg")
            ;;
        wow64aud)
