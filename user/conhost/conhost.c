...
/* proskrnl (M9): the wineserver seam is replaced by the kernel ConDrv
 * server transport - see proskrnl_glue.h. Everything else in this file is
 * the pinned Wine conhost, unmodified. */
...
{
    /* proskrnl: the kernel transport's read-completion write stands in for
     * wineserver's get_next_console_request(read=1). */
    if (console->read_ioctl == IOCTL_CONDRV_READ_CONSOLE_CONTROL)
        status = proskrnl_console_read_complete( console->server, status, signal,
                                                 &console->key_state, sizeof(console->key_state),
                                                 buf, size );
    else
        status = proskrnl_console_read_complete( console->server, status, signal,
                                                 NULL, 0, buf, size );
...
static NTSTATUS process_console_ioctls( struct console *console )
{
    ...
    for (;;)
    {
        ...
        /* proskrnl: reply-and-fetch through the kernel transport instead of
         * wineserver's get_next_console_request. */
