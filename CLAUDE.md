...
**Status: M3 complete; M4 next.** Boots via Limine, own page tables + pool, kernel threads with a 32-level priority scheduler (one dispatcher lock, no preemption — Art. 3), dispatcher objects and `KeWaitFor*` (M2), and the **object manager** (M3): refcounted object headers + type system, growable handle table, `\`-rooted namespace (`\Device`, `\??`, `\BaseNamedObjects`, directories, symlinks, case-insensitive/OPENIF/relative opens), with M2's dispatcher objects moved under Ob (`NtCreateEvent`/`NtOpen*`/`NtClose`/`NtDuplicateObject`/`NtWaitFor*`, kernel-callable until M4). **Minimal KASAN** (pool shadow + `__asan_*` hooks + red zones) is always compiled in. `abi/` (`ntstatus.h`, `ntdef.h`, `ntobapi.h`) is generated by `tools/gen_abi.py` (`make gen-abi`). `make run` → `[KTEST] M3 PASS` (in-kernel `tests/kmt`); `tests/run/run.sh oracle` → `sem_ob/` green on Wine. Build with **Make** (ADR 0009), boot via **Limine** (ADR 0010); toolchain in the README "Prerequisites". M4 is user mode + the syscall boundary (`docs/02`). *(Keep this line current — see Workflow.)*
