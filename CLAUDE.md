...
**Status: M4 complete; M5 next.** Boots via Limine, own page tables + pool, kernel threads with a 32-level priority scheduler (one dispatcher lock, no preemption — Art. 3), dispatcher objects and `KeWaitFor*` (M2), and the **object manager** (M3): refcounted object headers + type system, growable handle table, `\`-rooted namespace (`\Device`, `\??`, `\BaseNamedObjects`, directories, symlinks, case-insensitive/OPENIF/relative opens). **M4 is user mode + the syscall boundary:** own GDT/TSS + `syscall`/`sysret` entry (`kernel/syscall/`, generated numbers via `tools/gen_syscalls.py`), per-process address spaces (`Ps` + `EPROCESS`, kernel PML4 frozen and shared by copy), the handle table moved into the process, `Nt{Allocate,Free,Query}VirtualMemory` (reserve/commit VADs, `kernel/mm/virtual.c`), TEB allocation (`NT_TIB`), `NtTerminateProcess`/`NtDisplayString`, user-pointer probing (`kernel/syscall/uaccess.c`), and containment of a user fault as process termination. Flat-binary user clients run as Limine boot modules (`user/init-tests/`). **Minimal KASAN** (pool shadow + `__asan_*` hooks + red zones) is always compiled in. `abi/` (`ntstatus.h`, `ntdef.h`, `ntobapi.h`, `ntmmapi.h`, `ntpsapi.h`, `syscall_numbers.h`) is generated by `tools/gen_abi.py` + `tools/gen_syscalls.py` (`make gen-abi`). `make run` → `[KTEST] M4 PASS` (in-kernel `tests/kmt` + the ring-3 boot modules); `tests/run/run.sh oracle` and `tests/run/run.sh proskrnl` → `sem_wait/`, `sem_ob/`, `sem_mm/` green on Wine **and** on the kernel via syscalls. Build with **Make** (ADR 0009), boot via **Limine** (ADR 0010); toolchain in the README "Prerequisites". M5 is sections + image mapping (`docs/02`). *(Keep this line current — see Workflow.)*
