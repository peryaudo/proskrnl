...
| **Name case folding** (M3) | `OBJ_CASE_INSENSITIVE` honoured per lookup, ASCII **and the Latin-1 Supplement** (`tests/ntapi/sem_file/name_case_fold.c`) | beyond Latin-1 the fold is the identity; NT carries a full 64 KiB upcase table and proskrnl carries a rule. The ASCII-only version was recorded here as "unobservable until user mode invents non-ASCII object names" — `ntdll:directory` invented them (it creates U+00E9 and U+00C9 in one directory and requires the enumeration sort to treat them as one letter, `directory.c:324`), so the fold was widened to Latin-1 with its three irregularities (U+00DF and U+00F7 fold to themselves; U+00FF folds to U+0178, **not** U+00DF). Greek/Cyrillic/etc. remain unfolded and unobserved |
