...
| Debug objects | 6 | `NtCreateDebugObject` `NtDebugActiveProcess` `NtDebugContinue` `NtRemoveProcessDebug` `NtWaitForDebugEvent` `NtSetInformationDebugObject` | Decided post-CUI-4: no baked consumer (a debugger is a tool, not an app), and native Windows debuggers expect PDB symbol flow where proskrnl's toolchain is DWARF end-to-end (`docs/03` "Debug objects") |
